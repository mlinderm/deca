...
private[cli] object DecaMain {

  def main(args: Array[String]) {
    new DecaMain(args).run()
  }
}

private class DecaMain(args: Array[String]) extends Logging {

  private val commands: List[BDGCommandCompanion] = List(Normalizer, Coverager, Discoverer, CNVer)

  private def printVersion() {
    val about = new About()
    println("\nDeca version: %s".format(about.version()))
    if (about.isSnapshot()) {
  ...
  def run() {
    log.info("Deca invoked with args: %s".format(args.mkString(" ")))
